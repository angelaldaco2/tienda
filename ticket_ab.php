...
$transaccion=mysql_fetch_array(mysql_query("SELECT ID_Apartado id FROM Abono ORDER BY ClaveAbono DESC LIMIT 1")) or die ("Error:Al obtener la El abono".mysql_error());//$_REQUEST[transaccion];
...
$pdf=new PDF_AutoPrint('P','mm',array($ancho_papel,$alto_papel));
    ...
    while ($reg=mysql_fetch_array($registros))
    {
        ...
        $registros3=mysql_query("select * from abono where ID_Apartado ='". $transaccion ."'") or die("Error:4".mysql_error());
            ...
            $chars_max = $ancho_precio+10;
            $pdf->Ln(5);
            $pdf->MultiCell($ancho_celda,5,"Abonos",0,'C');

            $pdf->Cell($ancho_cantidad,5,"#",0,0,'C');
            $pdf->Cell($ancho_descripcion,5,"Total",0,0,'C');
            $pdf->Cell($ancho_precio,5,"Fecha",0,0,'C');
        ...
        while ($reg3=mysql_fetch_array($registros3)){
            ...
            $MT = $reg3['MontoTarjeta'];
            $MM = $reg3['MontoMonedero'];
            $TotalAb= $ME+$MT+$MM;
        ...
        // Se ponen los conceptos de compra
            ...
            $pdf->Cell($ancho_cantidad,5,$Abono,0,0,'C');
            $pdf->Cell($ancho_descripcion,5,$moneda.number_format($TotalAb,2,".",","),0,0,'C');
            $pdf->Cell($ancho_precio,5,$FechaA,0,0,'C');
        ...
        $pdf->Cell($ancho_cantidad,5,"",0,0,'C');
        $pdf->Cell($ancho_descripcion,5,"TOTAL APARTADO:",0,0,'L');
        $pdf->Cell($ancho_precio,5,"",0,0,'L');
        $pdf->Cell($ancho_precio,5,$moneda.number_format($TotalA,2,".",","),0,1,'R');
        if ($Estado='Pendiente')
        //Poner el SALDO Y FECHA DE VENCIMIENTO
        $pdf->Cell($ancho_cantidad,5,"",0,0,'C');
        $pdf->Cell($ancho_descripcion,5,"SALDO:",0,0,'L');
        $pdf->Cell($ancho_precio,5,"",0,0,'L');
        $pdf->Cell($ancho_precio,5,$moneda.number_format($Saldo,2,".",","),0,1,'R');
        $pdf->Cell($ancho_cantidad,5,"",0,0,'C');
        $pdf->Cell($ancho_descripcion,5,"FECHA VENCIMIENTO:",0,0,'L');
        $pdf->Cell($ancho_precio,5,"",0,0,'L');
        $pdf->Cell($ancho_precio,5,$FechaF,0,1,'R');
        ...
        $pdf->SetFont('Arial','',6);
        $pdf->Cell($ancho_celda,5,"(En apartados no hay devoluciones)",0,1,'C');
        $pdf->Ln(5);
        $pdf->SetFont('Arial','',8);
        if ($ID_Cliente!=1)
        {$pdf->Cell($ancho_celda,5,utf8_decode( $Cliente),0,1,'C');}
        $pdf->Cell($ancho_celda,5,utf8_decode("¡Gracias por su preferencia!"),0,1,'C');
